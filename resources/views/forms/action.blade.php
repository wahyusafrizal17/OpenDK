<div class="btn-group hidden-xs" id="exampleToolbar" role="group">
    @if( isset( $detail_url ) )
        <a href="{!! empty( $detail_url ) ? 'javascript:void(0)' : $detail_url !!}" class="{!! empty( $detail_url ) ? 'disabled' : '' !!}" title="Detail" data-button="detail">
            <button type="button" class="btn btn-info btn-xs" style="width: 40px;"><i class="fa fa-list fa-fw"></i></button>
        </a>
    @endif
    @if( isset( $show_url ) )
    ...
    @if( isset( $download_url ) )
        <a href="{!! empty( $download_url ) ? 'javascript:void(0)' : $download_url !!}" class="{!! empty( $download_url ) ? 'disabled' : '' !!}" title="Unduh" data-button="download">
            <button type="button" class="btn btn-info btn-xs" style="width: 40px;"><i class="fa fa-download"></i></button>
        </a>
    @endif
        @if( isset( $agree_url ) )
